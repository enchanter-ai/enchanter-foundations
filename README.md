...
- **Self-test fixtures.** [`docs/self-test.md`](docs/self-test.md) ships the A/B fixture methodology; **19 of 19 modules** have shipped fixtures as of 2026-05-06 (complete inventory coverage). On **Sonnet 4.6** as subject: 5 of 19 show clear behavioral delta; 12 show none (training contamination — baselines often cite framework terms unprompted); 1 v2 redesign showed **inverse delta** (treatment over-thought tools, scored worse than baseline — most interesting cross-batch result). On **Haiku 4.5** as subject (8 of 14 Sonnet-no-delta modules sampled): **5 of 8 show measurable behavioral delta** — strong on `failure-modes`/`verification`/`doubt-engine`, clear on `tool-use`/`delegation`, marginal on others. **The contamination hypothesis is supported.** The honest claim: *modules act as documentation of behavior on Sonnet and as runtime guidance on Haiku.* See [`docs/self-test.md`](docs/self-test.md) § Haiku-tier rerun for the full reading. [`recipes/stupid-agent-review.md`](recipes/stupid-agent-review.md) ships the runtime architecture; [`tests/runner.py`](tests/runner.py) ships a reference Python runner (~280 lines, anthropic SDK + stdlib); [`.github/workflows/self-test.yml`](.github/workflows/self-test.yml) ships an example CI workflow.
