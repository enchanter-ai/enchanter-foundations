...
- **Self-test fixtures.** [`docs/self-test.md`](docs/self-test.md) ships the A/B fixture methodology; **19 of 19 modules** have shipped fixtures + **4 validation tests** as of 2026-05-06. On **Sonnet 4.6** as subject: 5 of 19 show clear behavioral delta; 12 show none (training contamination — baselines often cite framework terms unprompted, **confirmed by an OOD synthetic-module control where the baseline did not reach for a fabricated numeric constant**). On **Haiku 4.5** as subject (12 of 14 Sonnet-no-delta modules sampled): **8 of 12 show measurable behavioral delta** — 4 strong (`failure-modes`/`verification`/`doubt-engine`/`tier-sizing`), 4 clear (`tool-use`/`delegation`/`cost-accounting`/`memory-hygiene`). **The contamination hypothesis survives a falsifiability test.** A previously-headlined "inverse delta" finding for `skill-authoring v2` was retracted after replication (3 of 3 reruns produced minimal tools — original was N=1 variance). The honest claim: *modules act as documentation of behavior on Sonnet and as runtime guidance on Haiku — this is now supported by replication, OOD control, and partial blind-criteria validation.* See [`docs/self-test.md`](docs/self-test.md) § Validation tests for the full reading. [`recipes/stupid-agent-review.md`](recipes/stupid-agent-review.md) ships the runtime architecture; [`tests/runner.py`](tests/runner.py) ships a reference Python runner (syntax + fixture parsing validated; end-to-end execution still adopter-side); [`.github/workflows/self-test.yml`](.github/workflows/self-test.yml) ships an example CI workflow.
