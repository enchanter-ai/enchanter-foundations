...
$VisRepo = if ($env:ENCHANTER_VIS_REPO) { $env:ENCHANTER_VIS_REPO } else { "https://github.com/enchanter-ai/vis" }
...
if (-not $Verify) {
    if (-not (Test-Path (Join-Path $VisDir ".git"))) {
        ...
        if ($LASTEXITCODE -ne 0) { Fail "clone failed — set ENCHANTER_VIS_REPO or clone manually" }
    }
