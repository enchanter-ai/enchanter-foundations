...
Audience: anyone reading this repo for the first time. The terminology used across conduct modules (in `packages/{core,skills,orchestration,safety,web,memory,cost}/conduct/`) and engines (`packages/orchestration/engines/`), defined once.
...
See [`../skills/conduct/skill-authoring.md`](../skills/conduct/skill-authoring.md) § Cross-vendor schema alignment.
...
A behavioral rule for agents. Lives in `packages/{core,skills,orchestration,safety,web,memory,cost}/conduct/`. Conduct *governs* — it says what to do or avoid. It does not compute.

## Engine

A math-grounded primitive an agent system leans on. Lives in [`../orchestration/engines/`](../orchestration/engines/). Engines *compute* — they return numbers or verdicts. They don't decide downstream actions; conduct does.

## Skill

A named workflow with a manifest (frontmatter) declaring its name, description, target model, and tool whitelist. The selector matches a user's request against the description; the body is read after selection. See [`../skills/conduct/skill-authoring.md`](../skills/conduct/skill-authoring.md).
...
A subagent on a lower tier that reviews a higher-tier agent's output. Catches shape errors and obvious mistakes before they propagate. The pattern's load-bearing form is documented at [`../skills/recipes/stupid-agent-review.md`](../skills/recipes/stupid-agent-review.md): a *cheap-tier* LLM running structural boolean tests against a higher-tier subject's artifact. The cost asymmetry is intentional — the verifier's literal mechanical reading is a feature, not a fallback.

## A/B fixture

A paired-run test that measures whether loading a conduct module changes agent behavior on a defined axis. Two identical runs — one without the module (baseline), one with (treatment) — scored against a binary or bounded pass criterion. Without the baseline run, you have not measured the module; you have measured the model. See [`../orchestration/docs/self-test.md`](../orchestration/docs/self-test.md) for methodology, [`../orchestration/tests/`](../orchestration/tests/) for shipped fixtures.
...
A low-tier subagent whose job is to fetch and extract from web pages. Owns `WebFetch`. See [`../web/conduct/web-fetch.md`](../web/conduct/web-fetch.md).
...
A prior distribution that, combined with a likelihood, yields a posterior in the same family. The Beta distribution is conjugate to the Bernoulli likelihood ([`../orchestration/engines/trust-scoring.md`](../orchestration/engines/trust-scoring.md)). Updates are `O(1)` — no integration required.
...
A statistical test that achieves the minimum expected sample size to decide between two hypotheses for fixed error rates ([`../orchestration/engines/sprt.md`](../orchestration/engines/sprt.md)). Use when you want to elevate a pattern only after enough evidence.
...
A maximal mutually-reachable subset of a directed graph. Found in `O(V + E)` by Tarjan's algorithm ([`../orchestration/engines/scc.md`](../orchestration/engines/scc.md)).

## LCS (Longest Common Subsequence)

The longest sequence appearing in both inputs while preserving order. Used to measure how much of an anchor sequence survives in a current state ([`../orchestration/engines/lcs-alignment.md`](../orchestration/engines/lcs-alignment.md)).
...
Minimum number of insert / delete / relabel operations to transform one tree into another ([`../orchestration/engines/tree-edit.md`](../orchestration/engines/tree-edit.md)). Quantifies structural change.

## Aho-Corasick automaton

A finite-state machine built from a fixed set of patterns; scans text in `O(|T| + z)` where `z` is the match count ([`../orchestration/engines/pattern-detection.md`](../orchestration/engines/pattern-detection.md)).
...
A measure of how "random" a string looks. Used to detect generated tokens (secrets, hashes) without a pattern list ([`../orchestration/engines/entropy-analysis.md`](../orchestration/engines/entropy-analysis.md)).

---

## All-Add

Anti-pattern memory strategy: appending every observation to working memory without pruning, leading to self-degradation as irrelevant or stale entries crowd out load-bearing ones. The correct alternative is selective, expiry-aware logging. See [`../memory/conduct/memory-hygiene.md`](../memory/conduct/memory-hygiene.md).
...
Base exception class in BAML's parser; subclasses indicate specific parse failure modes. See [`../skills/recipes/baml.md`](../skills/recipes/baml.md).

## DFA (Deterministic Finite Automaton)

A finite state machine used to evaluate temporal safety policies in [`../orchestration/engines/agentproof.md`](../orchestration/engines/agentproof.md). Given a sequence of agent actions, the DFA advances through states and flags sequences that violate a declared policy.

## Number of Flip

SYCON-Bench metric: how often a model shifts stance under sustained user pressure across a session. Complements Turn of Flip (which measures how quickly the first flip occurs). See [`../orchestration/conduct/multi-turn-negotiation.md`](../orchestration/conduct/multi-turn-negotiation.md) and [`../orchestration/engines/calibration.md`](../orchestration/engines/calibration.md).

## Turn of Flip

SYCON-Bench metric: how quickly a model conforms to user pressure — specifically, at which turn (counted from the first pushback) the model first abandons a well-founded position. Lower Turn of Flip indicates higher sycophancy risk. See [`../orchestration/conduct/multi-turn-negotiation.md`](../orchestration/conduct/multi-turn-negotiation.md) and [`../orchestration/engines/calibration.md`](../orchestration/engines/calibration.md).
